...
class DeepCRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        conv = conv.reshape(w, b, -1)     # [W, B, H*C]
        rnn_out, _ = self.rnn(conv)
        output = self.embedding(rnn_out)  # [W, B, nclass]
        return output
...
model = DeepCRNN(128, 1, nclass, 256)
model.load_state_dict(torch.load('deep_square_plate_crnn.pth', map_location='cpu'))
model.eval()
...
test_folder = r'C:\Users\PC\Desktop\plates\square_plates_for_test'  
results = []
correct = 0
total = 0
